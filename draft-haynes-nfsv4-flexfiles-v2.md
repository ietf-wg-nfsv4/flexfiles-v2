...
-  FFV2_ENCODING_MIRRORED, FFV2_ENCODING_MOJETTE_SYSTEMATIC,
   ...
   which carry the per-chunk CRC32 this version of the layout
...
-  Per-chunk integrity.  There is no CRC32 on the data path.
...
-  Per-chunk CRC32 on write and on read.  The CRC is computed
...
the crc32 carries a 32-bit CRC computed over the header and the
chunk.  Because the crc32 field is itself part of the header, the
...
stores the computed CRC into the crc32 field for transmission.
To validate on the read path, the receiver saves the received
crc32, treats those bytes as zero, recomputes the CRC over the
