...
That is a repurposing of the flag, and implementers should
understand it as one.  {{Section 13.1 of RFC8881}} determines a
...
path.  {{Section 13.1 of RFC8881}} does name a flag for a client
...
that is what leaves it free to mark the control session.

Presenting it does not conflict with {{RFC8881}}.
{{Section 13.1 of RFC8881}} permits a client to request zero or
...
:  On the reconstruction path,
   the repair actor MUST issue CHUNK_REPAIRED after CHUNK_COMMIT.  Without it,
...
3.  CHUNK_FINALIZE and CHUNK_COMMIT to persist the repaired shards.

4.  CHUNK_REPAIRED to clear the
    errored state.
...
2.  CHUNK_ROLLBACK on each affected
    shard to restore the previously committed content.

3.  CHUNK_UNLOCK on each shard.
...
3. The repair actor issues CHUNK_WRITE_REPAIR
...
5. The repair actor issues CHUNK_REPAIRED to
...
1. The repair actor issues CHUNK_LOCK on the
...
The wire lifecycle operations (CHUNK_COMMIT, CHUNK_FINALIZE, CHUNK_ROLLBACK) name generations by full
owner triple
...
-  TRUST_STATEID, REVOKE_STATEID, BULK_REVOKE_STATEID: the metadata-server-to-data-server tight coupling
...
see {{tbl-coding-types}}) is (chunked): it uses CHUNK_WRITE and CHUNK_READ and
...
: returned in exactly the cases where the data server can
  ...
  : the triple is malformed or truncated, or names a
    different file or a different data server.

  Within-window release:

  : a prior explicit CHUNK_ROLLBACK delete case released the
    ...
    triple as released-by-delete-case.
...
: covers every other case in which the data server holds no
  ...
  Retention-scope release:

  : release under the retention scope rule (lease expiry,
    storage-pressure release).

  Post-window release:

  : the eventual release of a triple invalidated by an
    earlier delete case whose replay-cache window has since
    elapsed.

  No surviving record:

  : a well-formed triple for this file and this data server
...
Returned by CHUNK_ESCROW_RELEASE when the escrow_id4
...
data server.  Clients MUST NOT SETATTR this attribute; a data
server MUST reject a client SETATTR of FATTR4_CHUNKED_DATA_FILE
...
in {{sec-data-file-identification}}.

The attribute describes the format of the file's content rather
...
misinterpretation is not detectable from the attribute alone.

The value is therefore fixed once the file holds data.  The
metadata server MUST NOT change the attribute on a file that is
not empty, and a data server MUST reject a SETATTR of
...
file MAY set the attribute to its final value at any point while
the file remains empty.  How a data server determines that a file
is not empty is an implementation matter.
...
:  When two or more clients race on the same chunk in the
   ...
   : a client that observes NFS4ERR_CHUNK_GUARDED on any
     ...
     transaction as lost.  It MUST issue CHUNK_ROLLBACK against every data server that
...
CHUNK_ESCROW_TAKEOVER accepts a bounded
...
CHUNK_ESCROW_TAKEOVER is the recovery
...
Like CHUNK_READ and CHUNK_WRITE, CHUNK_COMMIT carries an explicit
...
Like CHUNK_COMMIT, CHUNK_FINALIZE
...
:  A repair actor selected via CB_CHUNK_REPAIR walks the affected chunk
...
The lock is released by CHUNK_UNLOCK or implicitly when the holder's
...
A subsequent lifecycle operation (CHUNK_COMMIT, CHUNK_FINALIZE, CHUNK_ROLLBACK) that names an
...
Like CHUNK_COMMIT, CHUNK_ROLLBACK
...
({{fig-chunk-state-machine}}); a subsequent CHUNK_FINALIZE and CHUNK_COMMIT (or the activation shortcut
...
CHUNK_WRITE_REPAIR is the repair-path variant of CHUNK_WRITE.  It writes reconstructed chunk data
...
3.  The repair actor acquires a CHUNK_LOCK on the target data server to
...
5.  The repair actor issues CHUNK_FINALIZE and CHUNK_COMMIT to persist the repair.

6.  The repair actor issues CHUNK_REPAIRED to clear the errored state.
...
that owns the control session and uses tsa_client_id to authorize
cwa_client_id on CHUNK_WRITE and cg_client_id in any
chunk_guard4 CAS ({{sec-chunk_guard4}}).  A mismatch is rejected
with NFS4ERR_BAD_STATEID.  TRUST_STATEID is the mechanism by which
tight coupling (see {{sec-tight-coupling-control}}) is established
between the metadata server and the data server for a particular
layout.
...
After successful CHUNK_LOCK / ADOPT -> CHUNK_HEADER_READ:
