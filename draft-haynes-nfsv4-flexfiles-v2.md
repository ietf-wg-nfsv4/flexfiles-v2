...
Presenting it does not conflict with {{RFC8881}}.  Section 13.1
permits a client to request zero or more of the three role flags
even where the combination requested is contradictory, and
constrains only the combinations a server may return in its
eir_flags.  A storage device that
also serves a metadata server role, and that selects among its
roles from the requested ones, should account for this
document's use of the flag when composing its eir_flags.
