---
title: Trusted Stateid Control Protocol for the Flexible File Version 2 Layout Type
abbrev: FFv2 Trust-Stateid
docname: draft-haynes-nfsv4-flexfiles-v2-trust-stateid-latest
category: std
date: {DATE}
consensus: true
ipr: trust200902
...
keyword: [pNFS, flexfiles, trusted stateid, revoke-now]
...
venue:
  ...
  latest: https://ietf-wg-nfsv4.github.io/flexfiles-v2/draft-haynes-nfsv4-flexfiles-v2-trust-stateid.html
...
This document specifies the trusted-stateid control protocol
between the Metadata Server and Data Servers in the Flexible
File Version 2 Layout Type.  Three new NFSv4.2 operations
(TRUST_STATEID, REVOKE_STATEID, BULK_REVOKE_STATEID) replace
Flexible File v1's synthetic uid/gid fencing with revoke-now
admission control.  The Metadata Server remains the single
state authority; Data Servers are admission gates.  This
document also specifies the tight-coupling control protocol
(capability discovery, control sessions, crash recovery on
both sides) and the security considerations specific to
trusted stateids.
...
{{I-D.haynes-nfsv4-flexfiles-v2-requirements}} extends pNFS
with a control-plane protocol that replaces Flexible File v1's
synthetic uid/gid fencing with revoke-now admission control.
This document specifies:

- The Coupling of Storage Devices model: fencing, state and
  locking, and the tight-coupling control protocol between
  the Metadata Server and its Data Servers.
- The Layout Revocation and Fencing procedure.
- Three new NFSv4.2 operations (TRUST_STATEID,
  REVOKE_STATEID, BULK_REVOKE_STATEID) that the Metadata
  Server issues to Data Servers to push and clear per-client
  stateids.
- The Trusted Stateids Security Considerations.
...
Admission table:
:  A per-DS data structure holding the set of stateids for
   which the DS accepts I/O; entries are inserted by
   TRUST_STATEID and removed by REVOKE_STATEID (or all at once
   by BULK_REVOKE_STATEID).

Stateid-owner:
:  The client identified by a stateid registered in a DS's
   admission table.

Trust-stateid mechanism:
:  The mechanism by which the MDS pushes stateids to DSes
   before client I/O and revokes them before yielding the
   layout to another writer, providing revoke-now admission
   control without dependence on lease expiry.

Layout revocation window:
:  The bounded interval between REVOKE_STATEID fan-out from
   the MDS to all DSes in a layout and the DSes' rejection of
   subsequent I/O bearing the revoked stateid.
...
and ffv2ds_group in {{I-D.haynes-nfsv4-flexfiles-v2-layout}}), the client is provided
...
-  AUTH_SYS clients have no server-verified identity.  The
   ...
   ignored (see {{I-D.haynes-nfsv4-flexfiles-v2-layout}}).
...
{{I-D.haynes-nfsv4-flexfiles-v2-chunks}}), rather than dropping them.
...
:  A compromised data server sees plaintext chunk
   ...
   adversarial content ({{I-D.haynes-nfsv4-flexfiles-v2-chunks}}).
...
:  After REVOKE_STATEID or BULK_REVOKE_STATEID the data
   ...
   chunk_guard4 CAS ({{I-D.haynes-nfsv4-flexfiles-v2-chunks}}) bounds the
   ...
   ({{I-D.haynes-nfsv4-flexfiles-v2-chunks}}) prevents a write hole from
